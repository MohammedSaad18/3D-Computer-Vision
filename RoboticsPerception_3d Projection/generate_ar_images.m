...
render_points = [ corner_pts, zeros(4,1);
                  corner_pts, (cube_depth)*ones(4,1) ]; %hena howa kawn moka3b men ground plan we sa2f ertfa3ho el depth beta3 el mok3b
for i=1:num_frames
    % KLT Part
        draw_corners = {
      [corners(1,:,i) 5], ...
      ...
      [corners(3,:,i) 5], ...
      [corners(4,:,i) 5], ...
    };%el 5 deh 3lshan el radius beta3 el circle
    ...
    % video_pts
    p = (pr*(K \ [corners(:,:,i)'; ones(1,4)]))';%hena hwa hay7owl el corners men pixels le centimeters we ba3den ya5od awl safeen
    ...
    frame = frame(:, 1:2);
    frame;
    generated_imgs{i} = insertShape(generated_imgs{i},'Line',[frame(1,:) frame(2,:)],'Color','red');
    generated_imgs{i} = insertShape(generated_imgs{i},'Line',[frame(1,:) frame(3,:)],'Color','blue');       
    generated_imgs{i} = insertShape(generated_imgs{i},'Line',[frame(1,:) frame(4,:)],'Color','yellow');                    


    % Copy the RGB values from the logo_img to the video frame
    %generated_imgs{i} = draw_ar_cube(proj_pts,generated_imgs{i}); 
    
    %draw pyramid instead
    pyramid_pts=[corner_pts, zeros(4,1);0 0 cube_depth];
    pyramid = bsxfun(@plus,pyramid_pts * rot{i}',pos{i}') * K';
    pyramid(:,1)=pyramid(:,1)./pyramid(:,3);
    pyramid(:,2)=pyramid(:,2)./pyramid(:,3);
    pyramid = pyramid(:,1:2);
    pyramid;
    indss = [ 1,2, 1,4, 1,5, 2,3, 2,5, 3,4, 3,5, 4,5];
    X = pyramid(indss,:);
% Compute other positions
for j = 1:2:length(X)
    Xj = X([j,j+1],:);
    generated_imgs{i} = insertShape(generated_imgs{i}, 'Line', [round(Xj(1,1:2)), round(Xj(2,1:2)) ]);
end
